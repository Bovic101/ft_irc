...
//Method to validate channel user limit
bool Channel::validateLimit() const {
    return _channeluserLimit < 0 || _members.size() <= static_cast<size_t>(_channeluserLimit);
}
...
//Method to check if a mode is enabled
bool Channel::isModeEnabled(char mode) const {
    return _ChannelMode.find(mode) != _ChannelMode.end();
}

//To kick user from channel
void Channel::kickUser(int fd) {
    userDeletion(fd);
    deleteInvite(fd);
    revokeOperator(fd);
}
...
bool Channel::keyProtectedChecker() const {
    return isModeEnabled('k');
}
...
//This revoke channel key and disable key mode(k)
void Channel::revokeKey() {
    _channelKey.clear();
    disableMode('k');
}

//To check if user is operator to set topic(mode+t)
bool Channel::topicOperatorSetter(int fd) const {
    return checkOperator(fd) && isModeEnabled('t');
}

//To set topic by only if the user is permitted
void Channel::PermittedUserTopicSetter(int topicsetterFd, const std::string& topic) {
    if (topicOperatorSetter(topicsetterFd)) {
        topicSetter(topic);
    } else {
        throw std::runtime_error("Please, you are not permitted to set the topic.");
    }
}

//To upgrade user to operator
void Channel::upgradeUserToOperator(int fd) {
    if (checkUser(fd)) {
        giveOperator(fd);
    } else {
        throw std::runtime_error("User can not be found in the channel.");
    }
}

//To downgrade user from operator
void Channel::downgradeUserFromOperator(int fd) {
    if (checkOperator(fd)) {
        revokeOperator(fd);
    } else {
